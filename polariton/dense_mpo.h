...
namespace KylinVib
{
    ...
    class MPO : public std::vector<Dense<Type,4>>
    {
        ...
        void canon(double tol = 1e-8, int maxdim = 1000)
        {
            int ns = this->size();
        ...
        MPS<Type> diag_state() const
        {
            ...
            for(int i=0;i<ns;++i)
            {
                int bl = (*this)[i].shape()[0], br = (*this)[i].shape()[3],
                d = (*this)[i].shape()[1];
                Dense<Type,3> si({bl,d,br});
                for(int j=0;j<si.size();++j)
                {
                    std::array<int,3> sidx = si.make_indices(j);
