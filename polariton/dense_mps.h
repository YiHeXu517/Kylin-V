...
namespace KylinVib
{
    ...
    class MPS : public std::vector<Dense<Type,3>>
    {
        ...
        MPS(int ns, int nphys, std::map<int,int> const & cfg)
        : std::vector<Dense<Type,3>>(ns)
        {
            for(int i=0;i<ns;++i)
            {
        ...
        MPS(int ns, int nphys, std::vector<int> const & cfg)
        : std::vector<Dense<Type,3>>(ns)
        {
            for(int i=0;i<ns;++i)
            {
                Dense<Type,3> si({1,nphys,1});
                si({0,(int)cfg[i],0}) = 1.0;
                (*this)[i] = si;
            }
        }
        MPS(int ns, int nphys, int bd)
        : std::vector<Dense<Type,3>>(ns)
        {
        ...
        MPS(int ns, std::vector<int> const & nphys, int bd)
        : std::vector<Dense<Type,3>>(ns)
        {
            Dense<Type,3> s1({1,nphys[0],bd});
            s1.rand_fill();
            (*this)[0] = s1;
            Dense<Type,3> s2({bd,nphys[ns-1],1});
            s2.rand_fill();
            (*this)[ns-1] = s2;
            for(int i=1;i<ns-1;++i)
            {
                Dense<Type,3> si({bd,nphys[i],bd});
                si.rand_fill();
                (*this)[i] = si;
            }
        }
        ...
        void canon(double tol = 1e-14, int maxdim = 1000)
        {
            int ns = this->size();
            for(int i=0;i<ns-1;++i)
            {
                //auto[lef,rig] = svd<Type,2,1>((*this)[i],'r',tol,maxdim);
                auto[lef,rig] = svd<Type,2,1>((*this)[i],'r',0.0,1000,'n','y');
                (*this)[i] = std::move(lef);
                (*this)[i+1] = prod<Type,2,3,1>(rig,(*this)[i+1],{1},{0});
            }
            for(int i=ns-1;i>0;--i)
            {
                auto[lef,rig] = svd<Type,1,2>((*this)[i],'l',tol,maxdim,'n','y');
                (*this)[i] = std::move(rig);
        ...
        std::vector<LabArr<Type,2>> dominant(int nst) const
        {
            ...
            for(int site=1;site<this->size();++site)
            {
                ...
                {
                    int i1 = i / levs.size(), i2 = i % levs.size();
        ...
        std::vector<LabArr<Type,2>> dominant(double tol) const
        {
            std::vector<LabArr<Type,2>> levs = all_levels((*this)[0]), kepts;
            kepts = truncate_levels(levs,tol);
            for(int site=1;site<this->size();++site)
            {
                levs = all_levels((*this)[site]);
                std::vector<LabArr<Type,2>> envs(levs.size()*kepts.size());
                ...
                kepts = truncate_levels(envs,tol);
            }
            return kepts;
        }
        // product tools
        ...
        static std::vector<LabArr<Type,2>> all_levels(Dense<Type,3> const & mps)
        {
            ...
            {
                std::array<int,3> sidx = mps.make_indices(i);
        ...
        static std::vector<LabArr<Type,2>> truncate_levels(std::vector<LabArr<Type,2>> const & r, int nst)
        {
            if(r.size()<=nst)
            {
            ...
            [&tups](std::tuple<int,double> x1, std::tuple<int,double> x2){
        ...
        static std::vector<LabArr<Type,2>> truncate_levels(std::vector<LabArr<Type,2>> const & r, double tol)
        {
            std::vector<std::tuple<int,double>> tups;
            std::vector<LabArr<Type,2>> res;
            for(int i=0;i<r.size();++i)
            {
                tups.push_back( std::make_tuple(i,r[i].norm()) );
                if(r[i].norm()>tol)
