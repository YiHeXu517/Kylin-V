...
namespace KylinVib
{
  ...
  class Dense : public Allocator<Type>
  {
    ...
    Dense(std::array<int,Rank> const & Sp)
    : shape_(Sp), Allocator<Type>(std::accumulate(Sp.begin(),Sp.end(),1,std::multiplies<int>()))
    {
      dist_[Rank-1] = 1;
    ...
    Dense(std::initializer_list<int> Sp)
    : Allocator<Type>(std::accumulate(Sp.begin(),Sp.end(),1,std::multiplies<int>()))
    {
    ...
    Type & operator()(std::array<int,Rank> const & idx)
    {
    ...
    Type & operator()(std::initializer_list<int> idx)
    {
      int pos = std::inner_product(idx.begin(),idx.end(),dist_.begin(),0);
      if(pos>this->size())
      {
    ...
    Type const & operator()(std::array<int,Rank> const & idx) const
    {
      int pos = std::inner_product(idx.begin(),idx.end(),dist_.begin(),0);
      if(pos>this->size())
      {
    ...
    Type const & operator()(std::initializer_list<int> idx) const
    {
      int pos = std::inner_product(idx.begin(),idx.end(),dist_.begin(),0);
      if(pos>this->size())
      {
    ...
    std::array<int,Rank> const & shape() const
    {
      return shape_;
    }
    std::array<int,Rank> const & dist() const
    {
      return dist_;
    }
    std::array<int,Rank> make_indices(int idx) const
    {
    ...
    void print(double tol = 1e-14) const
    {
      ...
      for(int i=0;i<this->size();++i)
      {
        ...
        std::array<int,Rank> idx = make_indices(i);
        std::cout << "[";
