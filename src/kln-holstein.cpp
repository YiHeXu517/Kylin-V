...
int main( int argc, char ** argv )
{
    ...
    int op_ind = 0;
    string tole;
    Holstein::InputConfig Ir;
    while((ch=getopt_long(argc,argv,"i:t:h",long_op,&op_ind))!=-1)
    {
        switch (ch) 
        {
            case 'i':    
                Holstein::read_raw(optarg, Ir);
                break;
            case 't':
                tole = optarg;
                break;
            case 'h':
                std::cout <<  "Construct U(1)-symmetry conserved operators \n"
                          <<  "Usage: kln-holstein.a -i [file]\n"
                          <<  "Options:\n"
    ...
    // electronic states
    INT ns = Ir.SiteToLab.size();
    Holstein::all_ele_states(Ir);

    // all elementary operators
    vector<Holstein::Operator> RPO,LPO,RAO,LAO,TOT;
    ...
    // total Hamiltonian
    Holstein::totals(RPO,LPO,RAO,LAO,Ir,TOT,std::stod(tole));
    return 0;
}
