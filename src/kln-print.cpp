...
int main( int argc, char ** argv )
{
    ...
    while((ch=getopt_long(argc,argv,"i:t:h",long_op,&op_ind))!=-1)
    {
        switch (ch) 
        {
            ...
            case 't':
                ...
                break;
            case 'h':
                std::cout <<  "Print states or operators \n"
                          <<  "Usage: kln-print.a -i [file] -t [type,mps or mpo]\n"
                          <<  "Options:\n"
    ...
    ifstream ifs(fn);
    if(ftyp.find("mps")!=string::npos) { cout << Holstein::State::load(ifs) << endl; }
    else if(ftyp.find("mpo")!=string::npos) { cout << Holstein::Operator::load(ifs) << endl; }
